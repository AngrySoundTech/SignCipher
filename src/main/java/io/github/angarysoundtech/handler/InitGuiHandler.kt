...
object InitGuiHandler {
    ...
    fun onInitGuiEvent(event: GuiScreenEvent.InitGuiEvent) {
        ...
        if (gui is EditSignScreen) {
            ...
            event.addWidget(Button(gui.width / 2 - 100, maxY + 24, 200, 20, I18n.format("signpad.gui.encrypt")) { button ->
                ...
                if (match != null) {
                    ...
                    if (SignPad.library.pads.containsKey(key)) {
                        ...
                        for (i in 1..3) {
                            val encrypted = encrypt(tile.getText(i).formattedText, SignPad.library.pads[key]!!)
